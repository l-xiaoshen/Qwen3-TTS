...
class Qwen3TTSTokenizerV1DecoderDiTModel(Qwen3TTSTokenizerV1DecoderPreTrainedModel):
    ...
    def forward(
        ...
        # Compute embeddings
        time_embedding = cast(torch.Tensor, self.time_embed(time_step))
        text_embedding = cast(
            torch.Tensor,
            self.text_embed(
        ...
        text_embedding_unconditioned = (
            cast(torch.Tensor, self.text_embed(quantized_code, drop_code=True))
            if apply_cfg
            else None
        )
        ...
        position_embeddings = cast(
            tuple[torch.Tensor, torch.Tensor], self.rotary_embed(hidden_states)
        )
        blockwise_difference = self._create_block_diff(hidden_states)

        # Transformer blocks
        for transformer_block_module in self.transformer_blocks:
            transformer_block = cast(DiTDecoderLayer, transformer_block_module)
            hidden_states = cast(
                torch.Tensor,
                transformer_block(
                    hidden_states,
        ...
        hidden_states = cast(torch.Tensor, self.norm_out(hidden_states, time_embedding))
        output = cast(torch.Tensor, self.proj_out(hidden_states))
    ...
    def sample(
        ...
        def ode_function(
            time_step: torch.Tensor, hidden_states: torch.Tensor
        ) -> torch.Tensor:
        ...
        time_steps = time_embedding.unbind(0)
        for t0, t1 in itertools.pairwise(time_steps):
            dt = t1 - t0
